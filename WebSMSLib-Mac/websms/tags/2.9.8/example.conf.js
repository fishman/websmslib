...
Plugins["example"] = {
	...
	steps: [
		 ...
		 *	%FROM%				sender's sign (optional). You may probably append it to the %TEXT%.
		 *	%CCODE%				sender's international country code
		 *	%TO%				recipient phone number, with prefix
		 *	%TO_CCODE%			recipient international country code
		 *	%TEXT%				message text
		 *	%TO_INT_PREFIX%		recipient international prefix (currently not implemented)
		 *	%TO_PREFIX%			recipient phone prefix (in Italy, it's the first 3 cyphers)
		 *	%TO_NUMBER%			recipient phone number, without prefix
		 *	%CAPTCHA%			captcha code written by user
		 *
