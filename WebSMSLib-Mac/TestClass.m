...
- (IBAction) hud_btn_abortContinue:(id) sender {
	if ([hud_img_captchaCode isHidden] == NO) {
		[msg_engine setCaptchaCode: [hud_fld_captchaUser stringValue]];
		[msg_engine send];
		
		[hud_img_captchaCode setHidden: YES withFade: YES];
		[hud_img_captchaCode setImage: nil];
		[hud_fld_captchaUser setHidden: YES withFade: NO];
		[hud_fld_captchaUser setStringValue: @""];
...
- (void) debug_dataReceived:(NSString *) _data {
	...
	[[ActivityMonitorController sharedUIActivityMonitor] addNewLog: [log sanitizeString: [msg_engine replacements]]];
	NSLog(@"%@",log);
}
...
- (void) websms_requestCaptchaCode:(NSString *) captcha_src from:(WebSMSEngine *) engine {
	...
	[hud_fld_captchaUser setHidden: NO withFade: NO];
	[hud_fld_captchaUser setStringValue: NSLocalizedStringFromTable(@"UI_InsertCaptcha",@"Dialogs",@"")];
